...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (err) {
        toast(`Authentication failed: ${err.replace(/_/g, " ")}`, "error");
      }
    }
  }, [toast]);
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setSent(true);
      toast("Check your inbox for your sign-in magic link", "success");
    }
  };
        ...
        <div className="w-full max-w-md">
          <GlassPanel rounded="3xl" className="p-8 sm:p-10 text-center bg-[rgba(18,20,24,0.85)] border-white/10 shadow-[0_16px_48px_rgba(0,0,0,0.6)]">
            ...
            <p className="text-xs text-white/50 mb-8 max-w-xs mx-auto">
              Sign in to copy prompts, export generation parameters, and manage your portfolio.
            </p>
            ...
            <div className="mt-8 pt-5 border-t border-white/10">
              <p className="text-xs text-white/50">
                ...
                <Link href="/signup" className="text-white hover:text-[#FFB020] font-semibold transition-colors">
                  Sign up
                </Link>
              </p>
            </div>
